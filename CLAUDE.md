...
- **Tracker step configs**: `getTrackerSteps(state, lang)` and `getTrackerStepsV2(state, lang)` accept a language parameter and resolve translated strings via `t()`.
...
│   │   └── Footer (account switcher + consent, lang-aware, hidden during "sending")
